...
const Home = () => {
  ...
  const [todayPerYesterday, setTodayPerYesterday] = useState(0)
  ...
  const getPerToDay = ()=>{
    ...
    const yesterdayValue = (orders.filter((order) => {
      return (new Date(order.created_at).toISOString() < today.toISOString() &&  new Date(order.created_at).toISOString() > yesterday.toISOString())
    }).reduce((total, order) => total + order.total_order, 0))
    if(yesterdayValue === 0)
      setTodayPerYesterday(100)
    else
      setTodayPerYesterday(((todayOrder-yesterdayValue)/yesterdayValue)*100)
  }
  ...
  const getOrderToDay = () => {
    ...
    today.setMinutes(0);
    today.setSeconds(0);
    setTodayOrder(orders.filter((order) => {
      return new Date(order.created_at).toISOString() > today.toISOString()
    }).reduce((total, order) => total + order.total_order, 0))
  ...
  const getOrderYear = () => {
    ...
    setYearOrder(orders.filter((order) => {
      return new Date(order.created_at).toISOString() > lastyear.toISOString()
    }).reduce((total, order) => total + order.total_order, 0))
  }
        ...
        <div class="mb-12 grid gap-y-10 gap-x-6 md:grid-cols-2 xl:grid-cols-4">
          <div class="relative flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md">
            ...
            <div class="p-4 text-right">
              ...
              <h4 class="block antialiased tracking-normal font-sans text-2xl font-semibold leading-snug text-blue-gray-900">${todayOrder.toLocaleString()}</h4>
            </div>
            <div class="border-t border-blue-gray-50 p-4">
              <p class="block antialiased font-sans text-base leading-relaxed font-normal text-blue-gray-600">
                <strong class="text-green-500">+{todayPerYesterday}%</strong>&nbsp;than yesterday
              </p>
            </div>
          </div>
          {/* <div class="relative flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md">
          ...
          <div class="relative flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md">
            ...
            <div class="p-4 text-right">
              ...
              <h4 class="block antialiased tracking-normal font-sans text-2xl font-semibold leading-snug text-blue-gray-900">${yearOrder.toLocaleString()}</h4>
            </div>
            <div class="border-t border-blue-gray-50 p-4">
              <p class="block antialiased font-sans text-base leading-relaxed font-normal text-blue-gray-600">
                  ...
                  <tr>
                    ...
                    <td class="py-3 px-5 border-b border-blue-gray-50">
                      <div class="w-10/12">
                        ...
                        <div class="flex flex-start bg-blue-gray-50 overflow-hidden w-full rounded-sm font-sans text-xs font-medium h-1">
                          <div class="flex justify-center items-center h-full bg-gradient-to-tr from-blue-600 to-blue-400 text-white w-full"></div>
                        </div>
                      </div>
                    </td>
                  </tr>

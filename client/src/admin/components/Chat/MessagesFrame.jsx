...
const MessagesFrame = ({ curentConversation, socket }) => {
    ...
    return (
        <div className=' col-span-8'>
            {customer &&
                <>
                    <div className=' w-full h-16 max-h-40 bg-lightBlue rounded-tr-3xl py-5 px-2'>
                        <p className='text-xl h-fit'>To: <span className=' text-black'>{customer.name_cus}</span></p>
                    </div>
                    <div className='px-10 mt-5 h-[460px] overflow-y-auto' ref={refMessages}>
                        {messages.map((message) => {
                            if (message.id_send !== customer.id_acc) {
                                return (
                                    <div className="chat chat-end">
                                        <div className="chat-bubble bg-blue-800 text-white">{message.content}</div>
                                ...
                                return (
                                    <div className="chat chat-start">
                                        ...
                                        <div className="chat-bubble bg-blue-gray-600 text-white">{message.content}</div>
                                        <div className="chat-footer opacity-50">
                                            <time className="text-xs opacity-50"><Moment fromNow>{message.timestamp.toString()}</Moment></time>
                                        </div>
                                    </div>
